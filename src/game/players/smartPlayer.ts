...
export class SmartPlayer extends Player {
  static highestExpectedValue = (
    dice: number[],
    scores: { [key: string]: number },
    rollsRemaining: number
  ): string => {
    const unscoredKeys = Scoring.scorables.filter(
      (scorable) => !(scorable in scores)
    )
    const expectedValues = unscoredKeys.map((key) => {
      return {
        key: key,
        value: Scoring.scorableExpectedValue[key](dice, rollsRemaining),
      }
    })
    return expectedValues.sort(
  ...
  playRound(
    ...
    const highestExpectedValueKey = SmartPlayer.highestExpectedValue(
      dice,
      scores,
      rollsRemaining
    )
    if (
      Scoring.countingScorables.includes(highestExpectedValueKey) &&
      rollsRemaining > 0
    ) {
      const diceIndeces = new Set(
        Scoring.scorableReRollStrategy[highestExpectedValueKey](dice)
      )
      return { diceIndeces }
    } else {
